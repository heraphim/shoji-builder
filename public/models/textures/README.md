...
Same round trip as `models/components` and `models/lamps`: the browser cannot
write into the project, so **Save** downloads a file and you drop it in here. The
listing (`index.json`) is served by the Vite plugin in `vite.config.ts` and is
re-read on every request in dev, so a file dropped in shows up without a restart.
