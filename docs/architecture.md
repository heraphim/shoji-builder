...
| `ShowcaseScene` | The realistic style: one bulb inside the shade, the paper shell around it, the room, the bloom, and a camera framed on the lamp once rather than on every slider frame. |
| `ShowcaseRoom` | The bedroom, out of boxes and two canvases: wall, ceiling, nightstand, bed, pillow, pendant and a hanging scroll. No downloaded models — see the file's own note on why. |
